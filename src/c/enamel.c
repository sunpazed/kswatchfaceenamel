...
GColor enamel_get_background(){
	Tuple* tuple = dict_find(&s_dict, 2183382614);
	return tuple ? GColorFromHEX(tuple->value->int32) : GColorFromHEX(0xaaaaaa);
}
// -----------------------------------------------------
...
#if defined(PBL_COLOR)
	if( key == MESSAGE_KEY_background) return 2183382614;
#endif
	return 0;
}
...
static void prv_inbox_received_handle(DictionaryIterator *iter, void *context) {
	if(dict_find(iter, MESSAGE_KEY_background)){
		if(s_dict_buffer){
			free(s_dict_buffer);
			s_dict_buffer = NULL;
		}
